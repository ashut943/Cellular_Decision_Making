#Example script for calculating the infromation metrics for the three cell system, in particular between the cells 3 and the tuple of cell 1 and 2 (together) (u3 and (u1,u2))
...
for h_error in h_error_array
    ...
    plots_folder="./plots/three_cell_results/"*type_of_boundary_condition*"/"*"N_$(N)_error_fix_$(error_str)"*"_"*type_of_optimisation*"/"*string(num_simulations)*"_"*string(num_timesteps)*"_two_v_one/"
    mkpath(plots_folder)
