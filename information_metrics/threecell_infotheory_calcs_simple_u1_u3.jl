#This is for calculation of informatin metrics of the three cell system (1,2,3) between cells 1 and 3
#Due to the symmetry of the system, the infomration metrics calculated here are no different from that between cell 1 and 2 (which is the other file)
...
include("infotheoryfuncs.jl")
include("../three_cell_coarsegraining/three_cell_coarse_graining_simple_x_z.jl")
...
function calculate_information_metrics_multiple_trajectories(threecell_system, starting_state_tuple; T=50.0, initial_state=1, num_simulations=100, num_timesteps=1000)
    ...
    println("Success trajectories 1: ", length(success_trajectories_1)) #number of trajectories where cell 1 was expressed and system desirably patterned
    println("Success trajectories 2: ", length(success_trajectories_2)) #number of trajectories where cell 2 was expressed and system desirably patterned
    println("Success trajectories 3: ", length(success_trajectories_3)) #number of trajectories where cell 3 was expressed and system desirably patterned
    println("Success trajectories: ", length(success_trajectories)) #number of trajectories where system desirably patterned
    println("Failed trajectories: ", length(failed_trajectories)) #number of trajectories where system patterned "incorrectly"
    
    ts = vcat(collect(range(0, T, num_timesteps)))#query infomration metrics at uniformly spaced time points
