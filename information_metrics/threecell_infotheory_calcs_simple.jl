...
include("../three_cell_experiments/three_cell_coarse_graining_simple.jl")
...
function calculate_information_metrics_multiple_trajectories(threecell_system, starting_state_tuple; T=50.0, initial_state=1, num_simulations=100, num_timesteps=1000)
    ...
    ni, np = CellularDecisions_final.varioussizes(N,3)
    ns=length(Tc);
    ...
    S_arr = [CellularDecisions_final.simulate_ctmc(Q, initial_state, T, N, 3, "boundary_2") for i = 1:num_simulations]
    ...
    Threads.@threads for trajectory_num in 1:length(S_arr)
        ...
        local ts_temp_, path_te_XY_temp, path_te_YX_temp, path_mi_temp, path_te_XY_rate_analytical_temp, path_te_YX_rate_analytical_temp, path_mi_rate_analytical_temp,path_te_XY_rate_temp,path_te_YX_rate_temp,path_mi_rate_temp= calculate_information_metrics_single_trajectory(threecell_system, trajectory, ts, starting_state_tuple)
