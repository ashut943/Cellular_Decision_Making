...
include("../information_metrics/infotheoryfuncs_three_cells_two_v_one.jl")
...
T = 30.0  # Time for simulations
num_simulations = 1000  # N number of simulations
num_timesteps = 10 # Number of timesteps for the information metrics, i.e number of queries
...
threecell_system_filename = generate_filename(folder_name,"threecell_system_"*type_of_optimisation_1)
threecell_system = CellularDecisions_final.load(threecell_system_filename)
plots_folder="./plots/three_cell_results/"*type_of_boundary_str*"/"*"N_$(N)_lambda_$(lambda_str)"*"_"*type_of_optimisation_1*"/"*string(num_simulations)*"_two_v_one/"
mkpath(plots_folder)
#--------------------------------
...
base_folder = joinpath(dirname(@__DIR__), "experiments", "three_cell_results", "Interior_point_method_results_"*type_of_boundary_condition)
folder_name = joinpath(base_folder, @sprintf("Interior_Point_Method_results_N_%d_lambda_%s", N, lambda_str))
threecell_system_filename = generate_filename(folder_name,"threecell_system_"*type_of_optimisation_2)
threecell_system = CellularDecisions_final.load(threecell_system_filename)
plots_folder="./plots/three_cell_results/"*type_of_boundary_str*"/"*"N_$(N)_lambda_$(lambda_str)"*"_"*type_of_optimisation_2*"/"*string(num_simulations)*"_two_v_one/"
mkpath(plots_folder)
#--------------------------------
# Get state matrices, sizes, and target states
statedict=threecell_system.state_dict
...
results_dict_local=calculate_information_metrics_multiple_trajectories(threecell_system, initial_state_array, T=T, initial_state=initial_state, num_simulations=num_simulations, num_timesteps=num_timesteps)
# calculate_information_metrics(twocell_system, string(N)*"_"*string(λ), initial_state_array, T=T, initial_state=initial_state, num_simulations=num_simulations, plot_path=plots_folder)
#now calculate the overall metrics
...
combined_plot_dir = "./plots/three_cell_results/boundary_comparison/"*type_of_optimisation_1*"_vs_"*type_of_optimisation_2*"_w_"*type_of_boundary_condition*"/"*"N_$(N)_lambda_$(lambda_str)"*"/"*string(num_simulations)*"_two_v_one/"
