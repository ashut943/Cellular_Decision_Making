#This is an example optimization script for the three cell system, for the globally optimal solution.
#This is for multiple warm starts, with short and long iterations
...
rho = [1, 1, 1]  # Cell 1 uses strategy 1, Cell 2 uses strategy 2, Cell 3 uses strategy 3
h_error = 0.02 # Target error rate
initial_tau_val =11.0 # Initial tau value for optimization
...
max_time=13.0 # maximum hitting time
...
initial_f_plus_array_0=[1.0 for i in 1:N];
initial_f_minus_array_0=[0.0 for i in 1:N];
initial_f_plus_array_1=[0.0 for i in 1:N];
initial_f_minus_array_1=[1.0 for i in 1:N];
initial_g_array=[1.0 for i in 1:N+1];
initial_k_off_array=[0.0]

initial_f_plus_array_0[1]=0.0
initial_f_plus_array_0[end]=1.0
initial_f_minus_array_1[end]=0.0
initial_g_array[1]=0.0
initial_g_array[2]=0.0
initial_g_array[3]=0.0
initial_g_array[4]=0.0
#--------------------------------
#++++++++++++++++++++++++++++++++
#--------------------------------
...
#++++++++++++++++++++++++++++++++
#--------------------------------
# First step: Run 100 random initializations with short iterations (10^4)
# Generate random initial conditions and collect converged solutions
#--------------------------------
num_warm_starts = 100
short_max_iter = 10000
long_max_iter = 100000
#--------------------------------

println("Step 1: running $num_warm_starts random initializations...")
println("--------------------------------")

# Storage for converged solutions from Stage 1
converged_solutions = []
converged_tau_values = []

for warm_start_idx in 1:num_warm_starts
    # Generate random initial P values in a box [0,1]^{5N+2}
    rand_f_plus_0 = rand(N) 
    rand_f_plus_1 = rand(N)
    rand_f_minus_0 = rand(N)
    rand_f_minus_1 = rand(N)
    rand_g = rand(N+1)
    rand_k_off = rand(1)
    
    # Apply boundary constraints
    rand_f_plus_0[1] = 0.0
    rand_f_plus_0[end] = 1.0
    rand_f_minus_1[end] = 0.0
    rand_g[1:4] .= 0.0
    
    rand_P_val_array = vcat(rand_f_plus_0, rand_f_plus_1, rand_f_minus_0, rand_f_minus_1, rand_g, rand_k_off)
    rand_P_dict = Dict(1=>rand_P_val_array, 2=>rand_P_val_array, 3=>rand_P_val_array)
    
    # Compute initial tau for this random initialization
    Q_rand = Q_maker(rand_P_dict, N, M_cell, statedict, statedictinv)
    Q_rand_absorbing = Q_absorbing_states_maker(Q_rand, all_targetstates)
    rand_tau_opt = hitting_time_mod(Q_rand_absorbing, targetstates_good, targetstates_bad, startstates, 0.0)
    
    try
        # Run solver with short iterations
        _, _, tau_result, P_result, status = run_nonlinear_solver_modified_upper_bound_speed(
            N, M_cell, K, max_time, rho, h_error, initial_state, 
            rand_P_dict, rand_tau_opt, 
            Dict(1=>false, 2=>false, 3=>false), type_of_boundary_condition;
            max_iter=short_max_iter
        )
        
        # Check if solver converged successfully
        if status == MOI.LOCALLY_SOLVED || status == MOI.OPTIMAL || status == MOI.ALMOST_LOCALLY_SOLVED
            push!(converged_solutions, P_result)
            push!(converged_tau_values, tau_result[initial_state])
            println("  Warm start $warm_start_idx: CONVERGED with tau = $(tau_result[initial_state])")
        else
            println("  Warm start $warm_start_idx: Did not converge (status: $status)")
        end
    catch e
        println("  Warm start $warm_start_idx: ERROR - $e")
    end
end

println("--------------------------------")
println("Completed: $(length(converged_solutions)) solutions converged out of $num_warm_starts")
println("--------------------------------")

#--------------------------------
#++++++++++++++++++++++++++++++++
#--------------------------------
# Second step: Re-run converged solutions with longer iterations
# take the best solutions from the first step and refine them
#--------------------------------
println("Second step: Now with longer iterations...")
println("--------------------------------")

refined_solutions = []
refined_tau_values = []

for (sol_idx, P_warm) in enumerate(converged_solutions)
    # Compute tau for this warm start solution
    Q_warm = Q_maker(P_warm, N, M_cell, statedict, statedictinv)
    Q_warm_absorbing = Q_absorbing_states_maker(Q_warm, all_targetstates)
    warm_tau_opt = hitting_time_mod(Q_warm_absorbing, targetstates_good, targetstates_bad, startstates, 0.0)
    
    try
        # Run solver with longer iterations
        _, _, tau_result, P_result, status = run_nonlinear_solver_modified_upper_bound_speed(
            N, M_cell, K, max_time, rho, h_error, initial_state, 
            P_warm, warm_tau_opt, 
            Dict(1=>false, 2=>false, 3=>false), type_of_boundary_condition;
        ...
        if status == MOI.LOCALLY_SOLVED || status == MOI.OPTIMAL || status == MOI.ALMOST_LOCALLY_SOLVED
            push!(refined_solutions, P_result)
            push!(refined_tau_values, tau_result[initial_state])
            println("  Refined solution $sol_idx: tau = $(tau_result[initial_state])")
        else
            # Keep original if refinement fails
            push!(refined_solutions, P_warm)
            push!(refined_tau_values, converged_tau_values[sol_idx])
            println("  Refined solution $sol_idx: Kept original (status: $status)")
        end
    catch e
        push!(refined_solutions, P_warm)
        push!(refined_tau_values, converged_tau_values[sol_idx])
        println("  Refined solution $sol_idx: ERROR, kept original - $e")
    end
end

println("--------------------------------")
println("Completed: $(length(refined_solutions)) solutions refined")
println("--------------------------------")

#--------------------------------
#++++++++++++++++++++++++++++++++
#--------------------------------
# Final step: Select best solution (lowest mean hitting time)
# Find the globally optimal solution from all refined candidates
#--------------------------------
println("Final step: selecting best solution...")
println("--------------------------------")

if isempty(refined_tau_values)
    error("No valid solutions found from warm starts!")
end

best_idx = argmin(refined_tau_values)
best_tau = refined_tau_values[best_idx]
P_opt_dict_global_temp = refined_solutions[best_idx]

# Compute tau for best solution
Q_best = Q_maker(P_opt_dict_global_temp, N, M_cell, statedict, statedictinv)
Q_best_absorbing = Q_absorbing_states_maker(Q_best, all_targetstates)
tau_opt = hitting_time_mod(Q_best_absorbing, targetstates_good, targetstates_bad, startstates, 0.0)

println("Best solution found at index $best_idx with tau = $best_tau")
println("--------------------------------")
println("Completed")
println("================================")
