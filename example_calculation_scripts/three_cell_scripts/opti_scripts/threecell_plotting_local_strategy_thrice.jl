...
folder_name_for_plots = joinpath(dirname(dirname(dirname(@__DIR__))), "plots", "three_cell_results", type_of_boundary_condition, "N_$(N)_error_fix_$(error_str)_local_strategy_thrice")
...
threecell_system_filename = generate_filename(folder_name,"threecell_system_local_suboptimal")
